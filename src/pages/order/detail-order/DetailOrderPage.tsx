        ...
        <Box sx={{ backgroundColor: '#F0F1F1', padding: '25px 30px' }} flex={1} display='flex' flexDirection='column'>
          ...
          <Box bgcolor="#fff" p={2} mb={2}>
                ...
                <TableBody>
                  {
                    order.orderDetails.map((orderDetail: any, index: number) => (
                      <TableRow key={orderDetail.productId}>
                        ...
                        <TableCell>{orderDetail.variant.imagePath && <img src={orderDetail.variant.imagePath[0]} alt={orderDetail.variant.name} style={{ width: '50px', height: '50px', objectFit: 'cover' }} />}</TableCell>
            ...
            <Box>
              ...
              <Typography variant="body1" sx={{ color: '#000' }}>{order.note.length === 0 ? "Không có" : order.note}</Typography>
            </Box>
          </Box>
          <Box bgcolor="#fff" p={2} mb={2}>
            ...
            <Typography variant="body1" sx={{ color: '#000' }}>Phương thức thanh toán: {order.paymentType === "CASH" ? "Tiền mặt" : null}</Typography>
          </Box>
        </Box>
      }
    </MainBox>
